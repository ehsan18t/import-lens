...
test("formatImportSize supports minimal, standard, and verbose display modes", () => {
  ...
  assert.equal(formatImportSize(result, { display: "standard", compression: "brotli", showWarnings: true }), "5.3 kB → 1.5 kB (br)");
  assert.equal(formatImportSize(result, { display: "verbose", compression: "brotli", showWarnings: true }), "5.3 kB min · 1.8 kB gz · 1.5 kB br · 1.6 kB zstd");
});
...
test("formatImportSize shows unavailable and warning states", () => {
  ...
  assert.equal(formatImportSize({ ...result, side_effects: true }, { display: "minimal", compression: "gzip", showWarnings: true }), "1.8 kB ⚠");
