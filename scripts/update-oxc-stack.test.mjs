...
test("parseUpdateArgs supports explicit versions and dry-run", () => {
  assert.deepEqual(parseUpdateArgs(["--oxc", "0.135.0", "--resolver", "11.22.0", "--dry-run"]), {
    dryRun: true,
    oxcVersion: "0.135.0",
...
test("updateOxcStack dry-run reports planned edits without writing files or lockfiles", async () => {
  ...
  assert.equal(result.oxcVersion, "0.135.0");
  assert.equal(result.resolverVersion, "11.22.0");
  assert.deepEqual(result.changedFiles.sort(), Object.values(repo.paths).sort());
  assert.deepEqual(writes, []);
  assert.deepEqual(execs, []);
  assert.match(await readFile(path.join(repo.root, repo.paths.cargoToml), "utf8"), /oxc_parser = "0\.134\.0"/);
});

test("updateOxcStack updates manifests, policy tests, SRS, config, and lockfiles", async () => {
  const repo = await tempRepo();
  const execs = [];
  ...
  const dependencyPolicy = await readFile(path.join(repo.root, repo.paths.dependencyPolicyTest), "utf8");
  ...
  for (const crate of oxcStackConfig.oxcCrates) {
    assert.match(cargoToml, new RegExp(`^${crate} = "0\\.135\\.0"$`, "mu"));
  }

  assert.match(cargoToml, /^oxc_resolver = "11\.22\.0"$/mu);
  assert.equal(manifest.dependencies["oxc-parser"], "0.135.0");
  assert.match(dependencyPolicy, /0\.135\.0/);
  assert.match(packageVsix, /0\.135\.0/);
  assert.match(srs, /0\.135\.0/);
  assert.match(srs, /11\.22\.0/);
  assert.match(config, /currentOxcVersion: "0\.135\.0"/);
  ...
  assert.deepEqual(execs, [
    ...
    ...oxcStackConfig.oxcCrates.map((crate) => [
      "cargo",
      ["update", "-p", crate, "--precise", "0.135.0"],
    ]),
  ]);
});
...
test("updateOxcStack resolves latest versions before editing", async () => {
  ...
  const result = await updateOxcStack({
    ...
    dryRun: true,
    fetchJson: async (url) => {
      if (url.endsWith("/oxc-parser/latest")) {
        return { version: "0.136.0" };
      }
...
test("updateOxcStack reports no changed files when target versions and scripts already match", async () => {
  ...
  const result = await updateOxcStack({
    ...
    dryRun: true,
    oxcVersion: "0.134.0",
    resolverVersion: "11.21.0",
    fetchJson: async (url) => {
      if (url.endsWith("/oxc-parser/0.134.0")) {
        return { version: "0.134.0" };
      }
      if (url.endsWith("/oxc_resolver/11.21.0")) {
        return { version: { num: "11.21.0" } };
      }
      const crate = oxcStackConfig.oxcCrates.find((crate) => url.endsWith(`/${crate}/0.134.0`));
      if (crate) {
        return { version: { num: "0.134.0" } };
      }
...
test("updateOxcStack rejects non-coordinated current OXC crates and oxc_mangler before edits", async () => {
  const nonCoordinated = await tempRepo({
    cargoToml: cargoTomlFixture().replace('oxc_ast = "0.134.0"', 'oxc_ast = "0.133.0"'),
  });
  const withMangler = await tempRepo({
    cargoToml: `${cargoTomlFixture()}oxc_mangler = "0.134.0"\n`,
  });
...
const availableVersionPayload = (url) => {
  if (url.endsWith("/oxc-parser/latest")) {
    return { version: "0.135.0" };
  }
  if (url.endsWith("/oxc-parser/0.135.0")) {
    return { version: "0.135.0" };
  }
  if (url.endsWith("/oxc-parser/0.136.0")) {
    return { version: "0.136.0" };
  }
  if (url.endsWith("/oxc_resolver")) {
  ...
  const crate = oxcStackConfig.oxcCrates.find((crate) => url.endsWith(`/${crate}/0.135.0`) || url.endsWith(`/${crate}/0.136.0`));
  if (crate) {
    return { version: { num: url.endsWith("0.136.0") ? "0.136.0" : "0.135.0" } };
  }
  throw new Error(`unexpected fetch: ${url}`);
};
...
brotli = "^8"
${oxcStackConfig.oxcCrates.map((crate) => `${crate} = "0.134.0"`).join("\n")}
...
const dependencyPolicyFixture = () => `assert.match(cargoToml, /^oxc_parser = "0\\.134\\.0"$/mu);
assert.match(cargoToml, /^oxc_resolver = "11\\.21\\.0"$/mu);
assert.equal(manifest.dependencies["oxc-parser"], "0.134.0");
`;

const packageVsixFixture = () => `const manifest = { dependencies: { "oxc-parser": "0.134.0" } };
...
const srsFixture = () => `| \`oxc_parser\`      | 0.134.0 |
| \`oxc_resolver\`    | 11.21.0 |
| \`oxc-parser\`                         | 0.134.0 |
currently resolved to 0.134.0
Currently resolved to 11.21.0
baseline v0.134.0
`;

const configFixture = () => `export const oxcStackConfig = {
  currentOxcVersion: "0.134.0",
  currentResolverVersion: "11.21.0",
};
`;
