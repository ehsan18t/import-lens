...
| --- | --- |
| root package cannot be resolved | **Unmeasured** under `package_resolution` / `package_manifest` / `entry_resolution` (FR-026d). A **declarations-only** package is the one exception, and it is not a failure: it resolves to no runtime entry *because it ships no runtime code*, and is answered **Measured** at zero, High confidence, with a `types_only` diagnostic. That distinction is load-bearing — treating it as an unresolved entry makes every file importing `@types/…` a permanent floor (FR-024a). |
