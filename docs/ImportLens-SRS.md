...
**FR-018a** (High) - **A package's non-JavaScript bytes are counted when they can be processed, and disclosed when they cannot.** A stylesheet, wasm module, or font that the package's own JavaScript entry imports is real shipped weight, and the Import Cost includes it (B2). The daemon must:
  ...
  * **intercept a shipped file even when it is not counted.** An image, icon or media file imported directly by JavaScript is stubbed like any other asset and disclosed at its real size. Left to Rolldown, one such import makes the WHOLE package unmeasurable: a `.png` is not UTF-8 and fails the loader, and a `.svg` *is* UTF-8 so it reaches the JavaScript parser and fails there. The interception list is an allowlist of extensions a bundler's file loader really emits — an unknown extension still falls through to Rolldown, because stubbing something we cannot name might stub real JavaScript;
