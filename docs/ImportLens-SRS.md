...
**FR-032a** (Critical) - **No verdict from a floor, and a gate that cannot measure must never report success.** A budget is judged against a complete size, so an import that has none, or whose numeric size is a disclosed request-local asset floor (`asset_io`/`compression`), is **not evaluated** — neither "pass" nor "fail". A File Cost carrying `incomplete` is likewise not evaluated, including a deterministic `uncounted_assets` omission. This applies to every surface that reaches a verdict: the Problems-panel diagnostics, the workspace report's violation counters, and `importlens check`.
