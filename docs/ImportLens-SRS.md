...
| `rolldown`        | 1.1.5                    | `=` exact pin  | ⚠️ No Rust-API semver | Optional dependency behind the non-default `rolldown-candidate` feature; never linked into the shipped binary today. Coordinated with the OXC stack through `pnpm deps:update:compiler` and the generated graph fingerprint (bundler redesign spec, qualification phase).         |
...
**C-003:** Rolldown now publishes an embeddable Rust crate (`rolldown` on crates.io), but its Rust API carries no semver or documentation guarantee. The production engine remains the custom module graph walker built on OXC primitives; `rolldown` is exact-pinned as an optional dependency behind the non-default `rolldown-candidate` Cargo feature strictly for the bundler-redesign qualification (docs/superpowers/specs/2026-07-10-bundler-redesign-design.md) and is never linked into the shipped binary today. It becomes the production engine only if every qualification gate in that design passes. See Appendix C: Technology Watch.
