...
6. Renders the size inline in the editor as native inlay hints, opt-in confidence-styled inline hints, end-of-line decorations, or code lens annotations
...
**FR-029** (Critical) - The extension must display size information inline by default through `importLens.display: "inlayHint"` and `importLens.inlineRenderer: "native"`. Native VS Code inlay hints are the default because they provide the most reliable hover target, editor integration, and screen-reader-accessible behavior. The colored inline renderer remains available through `importLens.inlineRenderer: "colored"` and uses decoration-backed inline text so individual imports can use confidence-specific colors. End-of-line text decorations remain available via `importLens.display: "standard"` or `importLens.display: "verbose"` for users who prefer line-end annotations.
...
**FR-036l** (Medium) - When `importLens.enableRegistryHints` is enabled, the extension may fetch npm metadata for registry-based hints. The setting must default to `false`; network work must use short timeouts, cache results in VS Code globalState, and fail silently without affecting size computation.
...
| `importLens.inlineRenderer`  | enum    | `native`    | Inline renderer for `display: "inlayHint"`: `native` VS Code inlay hints or `colored` decoration-backed hints |
...
| `importLens.enableRegistryHints` | boolean | `false`  | Opt in to short-timeout npm metadata hints cached in globalState                                          |
...
**FR-039d** (High) - When `importLens.display` is set to `inlayHint` and `importLens.inlineRenderer` is `colored`, the extension must register a source-range hover provider over each import statement range and return the same detailed `MarkdownString` used by native inlay hints and end-of-line decorations. This hover must be scoped to colored inlay rendering because decoration `after.contentText` is not a reliable standalone hover target in VS Code. Native inlay mode must continue to rely on the `InlayHint.tooltip` attached to the actual size label.

**FR-039b** (Medium) - The extension must include a note in its README and marketplace description that `importLens.inlineRenderer: "native"` is the default and preferred for screen-reader accessibility. End-of-line and decoration-backed inline renderers are not exposed to VS Code's accessibility APIs in the same way as native inlay hints. The native inlay-hint renderer uses the VS Code Inlay Hints API, which is part of the document model and is screen-reader-accessible. The status bar item (FR-033) must always reflect the current operating tier regardless of display mode, as it is accessible to screen readers.
...
| D-011 | Hybrid inline rendering                                                                   | VS Code native inlay hints are accessible, provide reliable size-label hovers, and integrate with editor controls, but the API cannot assign arbitrary colors per hint. ImportLens therefore defaults to native inlay hints and keeps decoration-backed colored inline hints available through `importLens.inlineRenderer: "colored"` for users who prioritize confidence colors.                                                                                              | Native InlayHints only (rejected: no per-hint confidence colors); colored decorations only (rejected: weaker hover target and accessibility); end-of-line decorations only (rejected: less inline and less accessible); CodeLens only (rejected: takes full line)            |
