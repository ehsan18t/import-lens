...
- A **direct** CSS, image, or other non-JS/TS import written in the user's own file as the thing being measured. (This is about what the lens annotates. Inside a measured package, a stylesheet/wasm/font its entry imports **is** counted, and other referenced kinds are disclosed — see FR-018a/FR-018b.)
...
10. Each completed import is written to `papaya` (memory) and `redb` (disk) only when its result and exact dependency observations are durable (FR-026c), and is pushed to the client on the `refreshed_results` channel as it lands. Once the last of them has landed, the daemon re-derives shared-module bytes across the whole document and pushes the corrections, since sharing is only knowable when every import of the file has been measured (FR-004a). Every one of these frames leaves through the same outbound channel as the responses themselves (FR-004c), so no request being served can hold them up.
...
**2026 revision — Rolldown adopted for linking and tree-shaking:** Rolldown now publishes an embeddable Rust crate (`rolldown` on crates.io). The custom module graph walker accumulated structural correctness defects (dangling generated bindings, dropped effectful initializers, silently merged ambiguous star exports, empty external re-export bundles), so the bundler-redesign qualification exercised Rolldown 1.1.5 against a committed construct matrix, pinned real packages, and absolute performance gates. That version number is the historical record of what was qualified on that date, not the current pin — §9.4 carries the current one, and the same gates are re-run on every rolldown bump. Every gate passed on 2026-07-11 (cold `css-tree/parse` p95 52.4 ms against a 500 ms gate; 20-import batch peak RSS 78 MB against a 400 MB gate; candidate ~1.9x faster than the legacy engine), and Rolldown replaced the custom walker as the only semantic bundler. Two corrections to that record, both from 2026-07-14: the suite carrying those gates was never invoked by CI (a *different*, legacy suite ran under a similar name), and the startup, cache-hit and idle-RSS gates had never been measured at all. Both are fixed — the gates run on every pull request and are now measured against the shipped daemon binary (startup 17 ms, cache hit 15 ms, idle RSS 21 MB) — and no gate was found to be failing. Because Rolldown's Rust API carries no semver guarantee, it is exact-pinned as part of the coordinated compiler stack behind one narrow adapter (see C-003 and §10.7).
...
**FR-004a** (Critical) - **An interactive document response must not wait for an engine build.** `AnalyzeDocumentResponse` must be returned as soon as the daemon has classified the document's imports against the cache: every import the cache can answer carries `status: "ready"` with its result, an import that resolves to no package carries `status: "missing"`, and an import whose engine build has not run carries `status: "loading"` with its resolved `request` and no result. The daemon must then build those imports off the response path and deliver each one to the client as it lands, on the existing `refreshed_results` push (`document_path` + index-aligned `identities` + the analysis generation). **A pushed identity is `specifier` + `import_kind` + `named` + `runtime`**, and every field is load-bearing: an Astro document can import the same package, with the same kind and the same named exports, from its frontmatter (Server) and from a client `<script>`, and those are two rows with two different sizes ([ADR-0005](adr/0005-a-runtime-is-an-artifact-boundary.md)) — without `runtime` in the key they collide and the client collapses them into one. The extension must render a `loading` import as a neutral "Calculating..." hint — never as zero bytes and never as an error — and must merge each pushed result into that import's state, dropping any push whose generation a newer analysis has superseded.
  ...
  **A replayed push must be captioned by the ANALYSIS, not by itself.** The insights that caption a size — over budget, the git working-tree delta, the shared-module note — are derived from inputs an analysis owns: the `git diff` it ran for that document, the history it read, the budgets in force. A push carries none of them; it can only capture what the extension happened to hold when it landed, which for a document mid-analysis *predates* the diff that analysis is captioned by. The store must therefore recompute the insights of a replayed push with the refiner of the `set` that is replaying it, never with the one the push arrived with — recomputing the whole document from the push's stale inputs takes the working-tree badge off every import in it.
  ...
  A streamed import result carries no `shared_bytes`: shared-module weight is a relation between two imports of the SAME document and the SAME runtime, so it is not knowable until the last of them has been measured. The daemon must therefore close a streamed document with one final `refreshed_results` push carrying the imports whose shared figure the response could not have had right — and must omit that push when nothing in the document is shared, since a zero shared figure and an absent one render identically.
...
4. The build must emit exactly one unminified ESM **chunk**; more than one means the graph was code-split, which cannot be measured from a single chunk without under-reporting the rest, and is a typed `output_shape` failure. Cycles link without duplicate module inclusion; dynamic imports inline into the single chunk (code splitting is disabled). A package's non-JavaScript files never reach the output as **assets** at all — Rolldown 1.2.0 fails the whole build at the link stage rather than emitting one — so the plugin records their bytes at `load` and links them as empty modules (FR-018a, Section 10.7).
...
**FR-018a** (High) - **A package's non-JavaScript bytes are counted when they can be processed, and disclosed when they cannot.** A stylesheet, wasm module, or font that the package's own JavaScript entry imports is real shipped weight, and the Import Cost includes it (B2). The daemon must:

  * link every classified asset as an empty module so the JavaScript graph can be measured at all. Rolldown 1.2.0 removed CSS bundling, and a `.css` module reaching it fails the **entire build** at the link stage (`UNSUPPORTED_FEATURE`). Every package whose ESM entry does `import './styles.css'` — most UI kits — was therefore unmeasurable, which nobody could see while a failed build was still given a fabricated size;
  * process the assets after the build: all reachable stylesheets bundle through Lightning CSS into **one** CSS artifact (its `@import` tree resolved and minified); wasm and fonts have no processor and ship as their raw bytes;
  * **deduplicate a stylesheet by resolved path, never by rule content.** A sheet two imports both reach — directly or through `@import` — contributes once, because that is what a bundler emits. Identical rules arriving from two *different* files are **not** merged: no bundler does that, so merging them would report a number nothing downloads. The accuracy oracle measures this shape directly (`scripts/accuracy-compare.mjs` builds a tree whose two children share a third sheet) and holds it against esbuild resolving the same tree. The degraded per-sheet path is the deliberate exception: it cannot dedupe, which is precisely why it reports `imprecise_assets` and forfeits its budget verdict;
  * **process assets per runtime group, and add across groups.** Each runtime's combined build has its own reachable set, so its stylesheets bundle into one artifact for that group and its binaries are compressed within it; the group totals are then summed. Nothing is deduplicated across a runtime boundary, for the reason ADR-0005 gives for JavaScript — the two payloads really do ship separately, so a shared stylesheet reached under two runtimes is weight the page pays twice;
  * compress **each artifact on its own** and add the results into the five reported sizes (ADR-0005). Never concatenate the JavaScript chunk and an asset before compressing: they ship as separate files;
  * report a per-kind `asset_breakdown` of `AssetContribution` (Section 10.2) so a fused number stays legible;
  * fall back rather than fabricate. Any asset the pipeline cannot process reverts to raw-byte disclosure under `uncounted_assets` — never below the pre-B2 behaviour;
  * **intercept a shipped file even when it is not counted.** An image, icon, media file or compiled native addon (`.node`) reached by the JavaScript graph is stubbed like any other asset and disclosed at its real size. Left to Rolldown, one such import makes the WHOLE package unmeasurable: a `.png` is not UTF-8 and fails the loader, a `.svg` *is* UTF-8 so it reaches the JavaScript parser and fails there, and a `.node` addon fails as the `.png` does with `UNLOADABLE_DEPENDENCY` — one `keytar.node` is why `@vscode/vsce` and `ovsx` reported no size at all. An addon is **disclosed and not counted** for the same reason an image is, and it is the reason FR-018b gives: its bytes really ship, they are outside the processed taxonomy, so a size that omits them is a floor and must say so. It is emphatically *not* the `external` criterion — an addon sits on disk inside the installed package, so it is shipped weight, not a runtime fetch. The interception list is an allowlist of extensions a bundler's file loader really emits, plus `.node`, which earns its place by the strongest form of the same argument — Node resolves that extension through `process.dlopen`, so such a file cannot be JavaScript by definition of its name. An unknown extension still falls through to Rolldown, because stubbing something we cannot name might stub real JavaScript. The rule is always the extension, never a named package;
  * hold a result at **Medium** confidence whenever anything is disclosed rather than counted. This is a decision, not an oversight: a number that omits bytes the user's bundle will really carry is not a High-confidence measurement of that package's cost, and claiming otherwise is the same overclaim — one order of magnitude smaller — that FR-026d exists to stop. It is also what the neighbouring `external` diagnostic already does for the same reason. Medium carries no `~` prefix (FR-031 reserves that for Low), so such a package reads as a plain number with a stated caveat, which is exactly what it is. A package whose assets were all counted cleanly emits no diagnostic and therefore **may reach High**.

**FR-018b** (High) - **Every resource a counted stylesheet references is counted, disclosed, or named — never dropped.** Lightning CSS reports each `url()` dependency against the source file that declared it, and the daemon must classify each one by what it *is* rather than by whether its extension happens to be countable:

  * a **local file of a counted kind** (wasm, font) is read, compressed as its own artifact, and counted once per canonical path — so a font reached both directly and through two stylesheets contributes once;
  * a **local file outside the counted taxonomy** (an image, an SVG) is disclosed under `uncounted_assets` at its real size. Its bytes ship, so a size that omits them is a floor and must say so. Classifying by countable-extension-or-nothing sent these out through a single silent path, which left the headline short by an undisclosed amount while still claiming High confidence;
  * a **local resource that cannot be located or inspected** — a public-root `/fonts/x.woff2`, an unreadable file, a reference whose percent-escapes do not decode to UTF-8, or a sheet whose URLs Lightning CSS could not inspect at all — is disclosed under `uncounted_assets` as an omission of unknown size. It is **not** `imprecise_assets`: those two stages point in opposite directions, and reporting missing bytes as an over-count prevents `incomplete` from firing (FR-024a);
  * a **runtime-fetched resource** — a CDN `@import`, a remote `url()`, or a **protocol-relative** `//host/path` in either — is disclosed under `external`. Those bytes are real weight for the page but are not shipped by this package, so the measured size is exact and **keeps its budget verdict**. Routing them through a precision stage silently disabled budgeting for every package that `@import`s a web font, and treating a protocol-relative reference as an unlocatable *local* file did the same thing one layer down: the number stayed correct while being labelled a floor.

  Exactly two references legitimately leave no trace, and they are decided before anything else is attempted: a `data:` payload, whose bytes are already inside the counted CSS text, and a bare fragment, which points at the current document. Everything else must reach one of the outcomes above — including a reference the daemon cannot interpret at all, and any reference left unexamined because the walk stopped early, both of which are named as omissions. Silence is not an outcome: a dropped reference leaves the result Measured at High confidence while its bytes are missing from the total.

  A resource that could not be located but *did* resolve to a path must retain that path in freshness (FR-025), so that supplying the missing file invalidates the result. A reference that never resolved to a path has nothing to fingerprint and is disclosed without one. A resolved path that was **absent** takes the absent-state fingerprint rather than the unverifiable one — a missing file is a deterministic fact about the package, and recording it as a transient I/O failure makes the package permanently non-durable and re-measured on every keystroke (FR-025, ADR-0006).
...
**FR-024a** (High) - CommonJS support is provided by Rolldown's link-time ESM/CJS interop, which is static analysis (the FR-024 prohibition on evaluation holds; Rolldown never executes package code). Named access into a CJS module works through interop binding at link time; a CJS package without granular module boundaries retains its whole library, which is the correct measured cost. Export enumeration for CJS entries reads the linked chunk's export list and therefore may surface only `default` even when `exports.name =` assignments are statically visible — the daemon must not guess additional names (Section 10.7's never-guess rule). An engine failure on a CJS entry is Unmeasured like any other (FR-026d); it is **not** sized from the entry file alone. File-level size requests must still return conservative totals with diagnostics rather than reporting zero bytes.
  ...
  * a Measured import whose `uncounted_assets` disclosure says supported shipped bytes are absent from its five sizes;
  ...
  **Any one of them makes the total a floor**, which must be flagged `incomplete`, returned anyway (a floor beats a zero) with a diagnostic naming the specifier and its stage, and **never** written to the L1 aggregate cache (FR-026c), persisted (FR-026c), or judged (FR-032a).
  ...
  A successful combined build is not automatically complete either. If its asset stage reports `uncounted_assets`, the JavaScript and healthy assets remain a useful measured floor, but the omitted supported bytes still set `incomplete`. `imprecise_assets` alone does not: the per-sheet CSS fallback counts every stylesheet and reads high, which is a separate disclosed quality issue rather than missing weight.
...
**FR-026c** (Critical) - **A non-durable outcome must never enter a durable store.** Durable means: the L1 memory and L2 disk import caches, the L1 aggregate file-size cache, every build memo, the extension's persisted histories, **and any pass/fail verdict** (FR-032a).
  ...
  **The gate is an allowlist over the failure stage, never `error` (FR-026d), and never a denylist.** A result may be written down only if its stage is a property of the package's **bytes** — the cache is keyed by those bytes' fingerprints, so such a fact expires exactly when it would change. A denylist of engine-loss stages is not sufficient: `entry_metadata`, `asset_io`, and `compression` describe a filesystem or machine condition, and under a partial denylist they can be cached until an unrelated manifest or analyzer revision changes. The allowlist refuses every stage nobody has classified, so the cost of forgetting is a rebuild, never a durable wrong answer (`pipeline::stage::may_enter_a_durable_store`).

  Three shapes are refused. A primary analysis that could not answer under a request-local stage (`timeout`, `panic`, `engine_gone`, `entry_metadata`, or direct `asset_io`). A *measured* result carrying a request-local diagnostic — either a failed full-package comparison, whose `truly_treeshakeable: false` is fabricated by the accident, or an asset floor carrying `asset_io`/`compression`. And any stage nobody has classified yet. Exact sizes may still exist in the second shape; the diagnostic is what prevents a partial or machine-dependent fact becoming durable.
  ...
  **And on the READ, not only the write.** A write-side gate protects a store from what it is handed today; it says nothing about what is already inside it. L2 outlives the process, so a row a build wrote before the gate existed is on real users' disks right now, and it would be decoded, served as a cache hit, and re-promoted into L1 forever. `DiskCache`'s read path therefore applies the same gate and **evicts** what it refuses. The cost is one rebuild.
  ...
  The build-derived stores cannot receive an `ImportResult`, which is enforced structurally (`scripts/test/result-model-guards.test.mjs`), but a successful artifact can still carry an unreadable-input sentinel. The shared full-package/export-list build memo therefore rejects an empty, conflicting, or unverifiable fingerprint set at its own insert boundary. The dependency-path index stores invalidation paths rather than an answer and needs no result-durability gate.
  ...
  * **`incomplete`** — a conservative per-import sum (FR-024a) that skipped a still-`loading` import carries `error: None` and no failing stage at all. The daemon must flag as `incomplete` any aggregate missing bytes, for any reason: an unmeasured contributor, a package that is **not installed**, or a successful build carrying `uncounted_assets` (FR-024a — a path alias that resolves to first-party source is not one of those, and `imprecise_assets` alone is an over-count, not a floor). A fallback sum must lift the missing import's stage and specifier into the aggregate diagnostics. The aggregate must be structurally incapable of adding the known bytes and dropping the missing-byte signal.
  ...
  * an `ImportResult` may be recorded only if it **has a size** and carries no request-local stage. Both conditions are load-bearing: the first catches the Unmeasured shapes, and the second catches measured comparison failures and partial asset floors (FR-026d — the two are NOT mutually exclusive);
  * a file total may be recorded only if it is `error`-free, not `incomplete`, not `degraded`, and carries no request-local stage of its own. Both flags are on the wire (`FileSizeDocumentResponse`, and the legacy `FileSizeResponse`) because the client cannot derive either: a still-`loading` import leaves no failing stage behind, and a failed combined build leaves every contributor looking perfect;
  ...
  **One rule, three processes, one drift check.** The daemon (`FileSizeComputation::is_cacheable`), the extension (`isDurableFileSize`) and the CLI (`isUsableFileSize`) each state this gate, because the CLI ships standalone and can import neither of the others — the same forced duplication as `pipeline::stage::TRANSIENT_ANALYSIS_STAGES`. They must consult the same fields, and a drift check must fail when one of them falls behind (`scripts/test/file-size-usability-coordination.test.mjs`). `degraded` was added to the first two and not the third, and the third is the one that issues CI verdicts.
...
**FR-026d** (Critical) - **A size exists if and only if a build succeeded.** Every import analysis is in exactly one of three states, and the state must be legible from the result's *type*, not from a convention a reader has to know (ADR-0006):
  ...
  * **Unmeasured** — no size, *ever*. The build could not answer, and the result carries the **stage** that could not answer it.
  ...
  2. **Outcomes split by cause.** *Deterministic* (`parse`, `link`, `missing_export`, `ambiguous_export`, `output_shape`, `module_graph_limit`, `oversized_entry`, `package_manifest`, `entry_resolution`, `package_validation`, `package_resolution`, `minify`) is a property of the package's bytes: same input, same outcome, so it may be cached. *Request-local* (`panic`, `timeout`, `engine_gone`, `entry_metadata`, `asset_io`, `compression`) is a property of this moment's scheduling, filesystem, or machine and says nothing reusable about the package. An unclassified stage joins the refused group by default. Only the deterministic group may enter a durable store (FR-026c). The product-wide request-local list lives in `pipeline::stage::TRANSIENT_ANALYSIS_STAGES` and is mirrored — under a drift check — in the extension and the CLI, which cannot import it; the durability allowlist over the whole vocabulary lives beside it in `pipeline::stage`.
  ...
  3. **The FABRICATED state must be unrepresentable**, not merely tested for. A fabricated state is a size no build produced. In the daemon this is enforced by construction: the size fields are private and the only two constructors are `ImportResult::measured(specifier, sizes)` and `ImportResult::unmeasured(specifier, stage, …)` — so a failing path cannot reach for a size, because the constructor that takes one is the one that does not take a stage.
     ...
     **A size together with a request-local stage is a different thing, and it is NOT unrepresentable — it is a real state.** A build can produce honest primary sizes while the *secondary* full-package comparison times out, or produce a disclosed partial asset size after an asset read/compressor failure. That result must exist (the user is owed the best available number and its cause), and no type can prevent it in any case: `diagnostics` is an open list whose `stage` is a string. The **runtime** gate keeps the result out of every store and verdict instead: it lives inside each store (FR-026c), not in a predicate a caller must remember.
  ...
  The request-local stage vocabulary is one list, owned by the daemon (`pipeline::stage::TRANSIENT_ANALYSIS_STAGES`) and mirrored in the extension and standalone CLI because the three cannot share a source; a drift check fails the build if they disagree.
...
**FR-031d** (Medium) - When multiple imports in the same file **and the same runtime** share module paths reported by `module_breakdown` and `shared_bytes`, the extension must add hover insight text naming up to three shared module basenames and the other specifiers that include them. An import in a *different* runtime that pulls the same module must never be named as a sharer: a runtime is an artifact boundary, so it ships its own copy and saves the others nothing ([ADR-0005](adr/0005-a-runtime-is-an-artifact-boundary.md)). If the daemon reports `shared_bytes` but the shared modules are outside the public top-module breakdown, the hover must still explain that shared bytes exist.
...
**FR-036i** (High) - **One file budget, one number.** The extension must support configured bundle budgets for per-import and per-file Brotli thresholds, surfaced as VS Code diagnostics and in inline/hover text.
  ...
  **One analysis generation issues ONE `FileSizeDocument` at a time, and at most one re-read.** The daemon supersedes an older size request for the same document and answers it `error`, and an errored size read withdraws the File Cost — so a second request for one generation blanks the status bar for a round trip on exactly the cold document the re-read exists to serve. A settle that lands *before* the analysis has issued its own read needs no read of its own (that read is about to see the settled document); a settle that lands *while* one is in flight waits for it rather than superseding it. The status bar shows a File Cost and must not name it a bundle size (FR-033).
  ...
  A File Cost that is `incomplete` or `degraded`, absent, or carries `imprecise_assets` yields **no verdict at all** — no violation and **no pass** (FR-032a). Cache/history durability is the base predicate (FR-026c); budgetability adds the stricter precision check because a deterministic upper bound is reusable without being safe to compare with a threshold.
...
**FR-032a** (Critical) - **No verdict from a floor or upper bound, and a gate that cannot measure must never report success.** A budget is judged against a complete, precise size, so an import that has none, whose numeric size is a disclosed request-local asset floor (`asset_io`/`compression`), or whose separately processed stylesheets carry the deterministic `imprecise_assets` upper bound is **not evaluated** — neither "pass" nor "fail". A File Cost carrying `incomplete` is likewise not evaluated, including a deterministic `uncounted_assets` omission; one carrying `imprecise_assets` is also not evaluated even though it remains cacheable. This applies to every surface that reaches a verdict: the Problems-panel diagnostics, the workspace report's violation counters, and `importlens check`.
...
| Requested named export missing from a package                       | Report the import **Unmeasured** under `missing_export`, with a diagnostic naming the export and the raw details kept in hover-copy output rather than inline UI. The daemon must not guess the binding the user asked for — a stubbed one answers a typo with a confident size. (This row read "return a normal `ImportResult` when partial sizing can continue" while two other sections of this document mandated zero-size semantics; the contradiction is resolved here in favour of Unmeasured.) |
| A dependency imports a binding another dependency does not export   | Measure the graph with that binding **stubbed**, and disclose it under `missing_export` at Medium confidence, naming the binding. The user asked for the package, not for that edge, so refusing the measurement reports nothing at all about a package that is otherwise entirely measurable. The number is a **floor**: binding to an export that existed would retain whatever implements it, so the size describes the graph *as installed*, not the graph a working version would build — the disclosure says so. `yuku-parser` dropped `walk` in a patch release while `rolldown-plugin-dts` still imported it, which made `tsdown` unmeasurable. Refused when the unmatched binding is the **requested** export (row above), and for `ambiguous_export`, which a stub cannot resolve. |
...
| Package ships CSS its JavaScript entry imports                      | Measure the JavaScript graph normally and link the stylesheet as an empty module. Rolldown 1.2.0 cannot bundle CSS (`UNSUPPORTED_FEATURE`, at the link stage), so left to it the **whole build fails** — which, before FR-026d, was invisible because the failure was silently given a fabricated size. Bundle the reachable stylesheets into one artifact, compress it separately, and add it to the reported sizes (FR-018a). A sheet that cannot be processed reverts to an `uncounted_assets` disclosure naming the files, at Medium confidence. |
| Counted stylesheet references an image or SVG via `url()`           | Disclose it under `uncounted_assets` at its real size and hold the result at Medium (FR-018b). Its bytes ship, so the number is a floor. Do not count it — image bytes are outside the processed taxonomy and counting them would change the oracle baseline. |
| Counted stylesheet references a CDN font or remote `@import`        | Disclose it under `external`. The measured size excludes it and is **exact**, so the result stays budgetable; only confidence drops. Do not treat it as a local omission or as an over-count. |
| Counted stylesheet's `url()` graph cannot be inspected              | Lightning CSS can fail the dependency-analysis print while both measuring prints succeed (an ambiguous relative `url()` in a custom property). Keep the CSS contribution, and disclose the undiscovered graph as an `uncounted_assets` omission — the sheet is counted but its referenced artifacts are missing, which is a floor, not an over-count. |
...
| ----------------- | ------------------------ | -------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc_parser`      | 0.140.0                  | `=` exact pin  | ✅ Stable API    | OXC monorepo crate. Must be upgraded in lockstep with the other OXC monorepo crates.                                                                                                       |
| `oxc_resolver`    | 11.24.2                  | `=` exact pin  | ✅ Stable        | Separate repo from OXC monorepo; versioned independently and upgraded separately.                                                                                                          |
| `rolldown`        | 1.2.0                    | `=` exact pin  | ⚠️ No Rust-API semver | The production semantic bundler since the Phase 3 cutover (with its `rolldown_common`/`rolldown_error` siblings at the same monorepo version). Every size-producing path links and tree-shakes through it. Coordinated with the OXC stack through `pnpm deps:update:compiler` and the generated graph fingerprint; every version bump re-runs the bundler-redesign qualification gates. |
| `oxc_semantic`    | 0.140.0                  | `=` exact pin  | ✅ Stable API    | Must match `oxc_parser` resolved version.                                                                                                                                                  |
| `oxc_minifier`    | 0.140.0                  | `=` exact pin  | ✅ Stable API    | Test every upgrade against the accuracy suite because minified output can shift across releases. The daemon uses the minifier result's scoping and private-member mappings for codegen.    |
| `oxc_codegen`     | 0.140.0                  | `=` exact pin  | ✅ Stable API    | Required for AST -> string. Use `minify: true`.                                                                                                                                            |
| `oxc_allocator`   | 0.140.0                  | `=` exact pin  | ✅ Stable        | Arena allocator. Must match parser resolved version.                                                                                                                                      |
| `oxc_span`        | 0.140.0                  | `=` exact pin  | ✅ Stable        | Source locations. Must match parser resolved version.                                                                                                                                      |
| `oxc_syntax`      | 0.140.0                  | `=` exact pin  | ✅ Stable API    | Syntax metadata used by the parser and downstream OXC stages. Must match parser resolved version.                                                                                          |
...
interface ImportResult {
  ...
  shared_bytes?: number;          // Raw bytes shared with another import in the same file AND runtime
  asset_breakdown: AssetContribution[]; // Per-kind non-JS weight already INSIDE the five sizes
}

// What each non-JavaScript kind adds to the sizes above (FR-018a). Each artifact was compressed on
// its own and its numbers added, so these rows sum into the headline rather than sitting beside it.
// Absent kinds are omitted; a package with no assets carries an empty list, never a zero row.
interface AssetContribution {
  kind: "css" | "wasm" | "font";
  raw_bytes: number;
  minified_bytes: number;         // for a binary artifact this equals its raw size
  gzip_bytes: number;
  brotli_bytes: number;
  zstd_bytes: number;
}
...
**A runtime is an artifact boundary, so every total — raw, minified, and compressed — is a sum across runtime groups.** Each group is minified and **compressed on its own**, and the results are added. Compressed bytes may be summed across an artifact boundary and never within one ([ADR-0005](adr/0005-a-runtime-is-an-artifact-boundary.md), [ADR-0004](adr/0004-import-lens-measures-imports-not-bundles.md)): the Server bundle and the Client bundle are two payloads that genuinely ship, each genuinely compressed alone, so adding their compressed sizes models reality. Compressing their *concatenation* once — the previous behavior — compresses away every byte of redundancy between two payloads that never meet, which made the reported compressed size a strict **lower bound** on what ships (measured at a **49% under-report** on a shared-heavy two-runtime Astro file) with nothing on the wire saying so. It is now the sum.

Only an Astro document has more than one group: every import of every other document is `Component`, and `Component` never co-occurs with `Server` or `Client` in one document. That is *why* the sum cannot over-report — for a single-runtime file it has exactly one term, and the number is unchanged.

When one runtime's build, minification, **or compression** fails, only that runtime's entries degrade to conservative non-deduplicated per-import sums with diagnostics; the other groups keep their real deduplicated, separately-compressed totals, and the file's totals are flagged `degraded` (never cached, never persisted, never judged). A failure in one group must never zero the file.
...
Cross-module linking and tree-shaking are owned by the embedded Rolldown bundler. The daemon does not implement module-graph construction, reachability analysis, binding renaming, namespace materialization, or ESM/CJS interop; the previous custom module graph walk algorithm was deleted at the bundler-redesign Phase 3 cutover. The current `ANALYZER_REVISION` and its `<engine>-<minor line>.x+<revision>` format are recorded authoritatively in `daemon/src/cache/key.rs`, with the bump rule beside them; [bundler-architecture.md](bundler-architecture.md) §13.5 summarizes how it gates the cache. Past values live in git history rather than in a hand-kept list.

One exception to the "does not implement" list above, recorded so it is not mistaken for dead code: the daemon **matches `package.json#sideEffects` globs against the entry it is measuring**, but that match is *reporting-only and retention-neutral*. It never reaches Rolldown and cannot change what is retained or what size is reported. It survives on the **successful measurement** path, where it decides the `side_effects` flag the UI shows beside the size, because Rolldown 1.2.0 does not expose its own retention decisions and there is no other way to tell the user whether the file they imported is one the package declared effectful. (It used to be described as surviving "solely on the static-fallback path". That path is deleted; the match is not, and it never depended on it.) See the I9 amendment in the bundler-redesign design.
...
This section specifies the engine boundary (`daemon/src/engine/`) that isolates Rolldown behind Import Lens-owned types. The construct matrix that qualified Rolldown, and which still gates every version bump, is `daemon/tests/candidate_matrix.rs`; the design rationale is [bundler-architecture.md](bundler-architecture.md).
...
- A failure never returns partially linked code for measurement, and a missing or ambiguous **requested** export is a typed `missing_export`/`ambiguous_export` failure with zero-size semantics — never a guessed binding. An unmatched binding **between two dependencies** is a different thing and is not refused: the user requested the package, not that edge, so it is stubbed, measured, and disclosed (see the failure-stage table below).
...
**Assets are intercepted by the PLUGIN, not emitted by Rolldown.** Rolldown 1.2.0 does not emit a CSS asset — it refuses the build outright at the **link** stage (`UNSUPPORTED_FEATURE: Bundling CSS is no longer supported`). So the plugin intercepts a classified non-JS module at `load`, captures it as an immutable byte snapshot with the fingerprint from that same read, and returns an empty module, which is what lets the JavaScript graph be measured at all. The pipeline then processes those snapshots after the build and folds their compressed sizes into the reported number (FR-018a). Any claim that an emitted asset is disclosed and the chunk measured beside it describes a mechanism this bundler does not have.

`uncounted_assets` is the **fallback** channel, not the outcome: it carries the bytes that could not be processed, or that were located but fall outside the counted taxonomy, or that could not be located at all (FR-018b). It is a floor — bytes are missing from the number. `imprecise_assets` is its opposite: every sheet counted, but shared bytes counted more than once, so the number reads high. Neither may be used for the other's shape.
...
**Failure policy.** **There is no static fallback.** Failures are typed by stage and produce an **Unmeasured** result under that stage (FR-026d) — no size, never a fabricated size, never the requested binding guessed, never a measurement of partially linked output, and never the entry file or the package directory measured in the bundle's place. **One symbol may be synthesized, and only this one:** an unmatched binding between two dependencies, stubbed so that an otherwise-complete graph can be measured at all. It is disclosed by name, it caps the result at Medium, and it never covers the export the user requested — that stays Unmeasured. `analyze_static_entry`, `approximate_directory_size` and `estimate_minified_source` are **deleted**; this table described them in the present tense long after they were gone, which is how the same defect kept being reintroduced.

**One build, several failures: the EARLIEST stage wins.** A build can fail in more than one module at more than one stage, and it must report the same stage every time it is run on the same bytes — the stage is the whole of what a user is told (FR-026d leaves no size to soften it) and a deterministic stage is *cached* (FR-026c), so a stage decided by chance is a durable wrong answer. Rolldown accumulates its diagnostics from module tasks it runs **concurrently**, so their order is a race: a package with a parse error in one module and an unresolved import in another reported `parse` on 38 runs and `resolve` on 10 of the same 48. The reported stage is therefore the **earliest present in pipeline order** — `module_graph_limit` → `resolve` → `parse` → `missing_export` → `ambiguous_export` → `link` → `generate` → `output_shape` — because the earliest failure is the likeliest root cause and the later ones are frequently its shrapnel, and because a new stage is then ranked by where the build reaches it rather than by someone's judgement of severity. The order is the declaration order of the stage vocabulary itself (`engine::stage`), so a stage cannot exist without a rank. The failure's message and its diagnostic list are ordered the same way, for the same reason: they are cached too.

**`module_graph_limit` leads the deterministic stages, and it is the one rank that is not a pipeline position.** A breach is not a stage the build reached — it is the build being *abandoned*. The graph blew a hard limit (Section 10.3: 2,000 modules, 20 MiB per module source, 100 MiB total), so it was never going to complete, and under FR-026d that is the entire reason the import has no size. A resolve error in one module of a graph that was thrown away is shrapnel: reporting it would hand the user the symptom and bury the cause, and it would be cached that way. So the breach preempts every module-level diagnostic, and the declared order now says so. It used to be ranked at the point the breach is *detected* — the plugin's `load` hook, which sits after `resolve` — which made the rank decorative and this sentence false: a package with an unresolvable relative import in one module and an oversized module in another reported `module_graph_limit` on 24 runs of 24, while the order above promised `resolve`. The adapter was right and the order was wrong. (The breach is not ranked *against* the diagnostics in practice — no Rolldown event kind maps to it, the limit being the daemon's own, so the adapter answers a recorded breach directly. The rank is what this document and the diagnostic ordering are derived from, which is exactly why it may not lie.)

The four request-local engine stages rank ahead of every deterministic stage above. `panic`, `timeout`, and `engine_gone` are constructed at the execution boundary, where module diagnostics do not exist. `asset_io` is observed in the plugin and deliberately preempts a Rolldown `resolve`/`output_shape` symptom: exact asset bytes were unavailable, so the symptom must not be cached as a package fact. The ordering makes the answer that is never cached win by default (FR-026c).
...
| engine cannot resolve an internal import | A **bare** specifier naming a subpath of another package becomes an import boundary: the graph that did bundle is measured, and the edge is disclosed on `external` at Medium confidence, naming the specifier and stating that what it would have pulled in is not in the size. This holds for every resolver refusal, not only "not found" — a package that ships a file and declines to export it (`PackagePathNotExported`) is the common case, and one such edge must not discard a whole package. A **path-like** specifier (`./`, `../`, absolute) is NOT covered: a package that cannot find its own relative file is broken, and **Unmeasured** under `resolve` is the honest answer. |
| `missing_export` on the **requested** export, or any `ambiguous_export` | **Unmeasured** — the daemon must not guess the binding the user asked for, and a stub cannot resolve a name lost to conflicting star providers |
| `missing_export` on an edge **between dependencies** | **Measured**, with that binding stubbed and disclosed under `missing_export` at Medium. Rolldown raises an unmatched import at error severity, so without this one broken edge several levels down discards a whole graph whose bytes are entirely known — one package dropping an export in a patch release would make every package above it unmeasurable |
...
| `asset_io` (a supported asset input could not be observed as exact bytes) | Direct-asset failures can make the import **Unmeasured**; CSS dependency failures can leave a disclosed partial size. Both carry `asset_io`, an unverifiable fingerprint, and are written to no cache/history and judged by no budget. |
...
| compression failure | Top-level compression can produce no size; per-asset compression can leave a disclosed partial size. **Not** durable: `flate2`/`brotli`/`zstd` fail on valid bytes only through allocation failure or IO — a condition of the machine, not of the package. |
...
| -------------------------------- | --------------------------- | ---------------------------------------------------------------------------------------- |
| `importLens.bundleImpactHistory.v3` | `BundleImpactHistoryItem[]` | Recent current-file total measurements shown by `Import Lens: Show Bundle Impact History`; v3 also excludes pre-D12 deterministic asset floors |
| `importLens.importCostHistory.v2`   | `ImportCostHistoryItem[]`   | Recent per-import measurements used to show trend notes in import hovers; v2 excludes unverifiable pre-D11 asset floors                 |
...
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `oxc_minifier`                            | Stable 0.x release line, currently resolved to 0.140.0. Produces 1-2% variance from SWC.                                                                       | New OXC releases; minifier API or output changes.                                                                                                  | Upgrade OXC crates as a coordinated batch; re-run integration suite to confirm no accuracy regressions.                                                                                                                           | Every OXC release     |
| `oxc_resolver`                            | Currently resolved to 11.24.2. Separate repository (`oxc-project/oxc-resolver`), versioned independently from the OXC monorepo. Currently on major version 11. | Major version bump (e.g. 12.x); breaking changes to `ResolverOptions` or the `resolve()` API.                                                      | May require `Cargo.toml` update and code changes in `resolver.rs`. Upgrade separately from the OXC monorepo batch and run integration suite before merging.                                                                       | Each release          |
