...
OXC Rust crates and the `oxc-parser` npm package use 0.x versions, but that does not mean they are alpha quality. OXC follows Rust and npm package versioning before a 1.0 line while publishing production-ready crates and packages. ImportLens intentionally keeps OXC ranges broad (`^0` for OXC monorepo crates and npm `oxc-parser`, `^11` for `oxc_resolver`) so `pnpm deps:update` can move to the latest compatible releases without rewriting manifests. The lockfiles provide reproducible builds between upgrade runs, and the Docker builder plus `rust-toolchain.toml` follow stable Rust so dependency MSRV bumps do not strand the build scaffold. The Docker cross-build toolchain also follows latest stable Zig and latest `cargo-zigbuild` by default, with exact build-arg overrides available only for emergency bisects. Minifier output can differ from SWC by 1 to 2 percent; that variance is acceptable for inline size estimates. See constraint C-001 in Section 13.1.

### 9.4 Dependency Manifest (Current Resolved Versions)

> **This table tracks the current resolved dependency versions and the intended upgrade ranges.** `Cargo.toml` and `package.json` should keep upgrade-friendly caret ranges while `Cargo.lock` and `pnpm-lock.yaml` provide reproducible builds. Run `pnpm deps:update` to intentionally refresh to the newest package set. OXC versioning policy last audited: **4 June 2026.**

#### 9.4.1 Rust Crates (`Cargo.toml`)

| Crate             | Current Resolved Version | Semver Range | Stability       | Notes                                                                                                                                                                                      |
| ----------------- | ---------------- | ------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc_parser`      | 0.134.0          | `^0`         | ✅ Stable API    | Broad caret range keeps the daemon on the latest OXC 0.x line. All OXC monorepo crates should resolve to the same version.                                                                |
| `oxc_resolver`    | 11.21.0          | `^11`        | ✅ Stable        | Separate repo from OXC monorepo; versioned independently.                                                                                                                                  |
| `oxc_semantic`    | 0.134.0          | `^0`         | ✅ Stable API    | Must match `oxc_parser` resolved version.                                                                                                                                                  |
| `oxc_transformer` | 0.134.0          | `^0`         | ✅ Stable API    | TS/JSX stripping only. Does NOT tree-shake.                                                                                                                                                |
| `oxc_minifier`    | 0.134.0          | `^0`         | ✅ Stable API    | Test every upgrade against the accuracy suite because minified output can shift across releases.                                                                                           |
| `oxc_mangler`     | 0.134.0          | `^0`         | ✅ Stable API    | Separate crate from `oxc_minifier`; keep resolved with the same OXC monorepo version.                                                                                                      |
| `oxc_codegen`     | 0.134.0          | `^0`         | ✅ Stable API    | Required for AST -> string. Use `minify: true`.                                                                                                                                            |
| `oxc_allocator`   | 0.134.0          | `^0`         | ✅ Stable        | Arena allocator. Must match parser resolved version.                                                                                                                                       |
| `oxc_ast`         | 0.134.0          | `^0`         | ✅ Stable API    | Shared AST data structures. Must match parser resolved version.                                                                                                                            |
| `oxc_span`        | 0.134.0          | `^0`         | ✅ Stable        | Source locations. Must match parser resolved version.                                                                                                                                      |
| `oxc_syntax`      | 0.134.0          | `^0`         | ✅ Stable API    | Syntax metadata used by the parser and downstream OXC stages. Must match parser resolved version.                                                                                          |
...
| ------------------------------------ | ---------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc-parser`                         | 0.134.0          | `dependency`                           | NAPI binding. `package.json` uses `^0` so lockfile refreshes can follow the latest OXC 0.x npm package. Must stay resolved to the same OXC monorepo release as the Rust-side `oxc_parser` crate. **NOT** `@oxc-parser/wasm` (deprecated).                                                                                                                                                                                                                                                                                                                                      |
...
| ------------------------ | ------------------------------------------------------------- | --------------------------------------------------------------- |
| `@oxc-parser/wasm` (npm) | Officially deprecated. No longer maintained.                  | `oxc-parser` (NAPI, `^0`)                                       |
...
**C-001:** OXC Rust crates and the `oxc-parser` npm package use 0.x versions, but those versions are not alpha releases. ImportLens keeps broad caret ranges (`^0` for OXC monorepo crates and npm `oxc-parser`, `^11` for `oxc_resolver`) so `pnpm deps:update` can follow the latest compatible package set. Size estimation accuracy of approximately plus or minus 2 percent remains acceptable for an inline hint tool. **Fallback strategy:** If `oxc_minifier` exhibits correctness regressions in the integration test suite after an upgrade, the team must pin to the last known-good version and file an upstream issue. No release VSIX will ship with a minifier version that fails the integration suite. As a last resort, the daemon may skip minification entirely and report only raw + compressed sizes, with a `(no-minify)` suffix on decorations.
...
| `oxc_mangler`                             | Stable 0.x release line, currently resolved to 0.134.0; separate crate from `oxc_minifier`.                                                        | Potential crate-boundary or API changes, including possible re-merge into `oxc_minifier`.                                                          | Minor import path changes if crates are merged.                                                                                                                                                                                   | Every OXC release     |
