...
- `sideEffects` is read from the manifest the resolver finds by walking **up to the package root** (the topmost manifest before the `node_modules` boundary). The supplied path is exactly that manifest, so the entry's `sideEffects` declaration is the package's own — which is what makes FR-021 true of the entry module, and it is the reason this metadata is supplied at all.
