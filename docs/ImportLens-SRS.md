...
**FR-004c** (Critical) - **The daemon's connection loop must be a pure multiplexer.** It does exactly three things: read frames, dispatch each request to a task, and write outbound messages. No handler may be awaited inside the loop body, and **every** response must leave through the same outbound channel as the unsolicited pushes — so that nothing an individual handler does can stall the delivery of a frame belonging to anything else. Responses keep their per-request correspondence (`request_id`), but there is no ordering guarantee *between* requests: a fast one may overtake a slow one, which is the point.
  ...
  **Ending the connection: cancel, then join under a bound, then flush — in that order.** Every task the loop spawns to serve a request or produce a push must be registered, so that shutdown, an idle recycle and a lost connection can account for it. Ending the connection must then:
  ...
  3. **Flush the cache unconditionally**, whether or not every task finished, after draining the frames those tasks queued on their way out.
...
**FR-024a** (High) - CommonJS support is provided by Rolldown's link-time ESM/CJS interop, which is static analysis (the FR-024 prohibition on evaluation holds; Rolldown never executes package code). Named access into a CJS module works through interop binding at link time; a CJS package without granular module boundaries retains its whole library, which is the correct measured cost. Export enumeration for CJS entries reads the linked chunk's export list and therefore may surface only `default` even when `exports.name =` assignments are statically visible — the daemon must not guess additional names (Section 10.7's never-guess rule). Engine failures on CJS entries fall back to conservative static entry sizing with structured diagnostics, and file-level size requests must return conservative totals with diagnostics instead of reporting zero bytes.
  ...
  A conservative total is a **floor**, not the file's size, and the daemon must be able to tell the difference. When a runtime group's combined build fails, its totals degrade to a sum of the per-import measurements the caller already holds — and two of those inputs may not be measurements at all: an import whose own build has not landed yet (`status: "loading"`, FR-004a) contributes exactly zero, and an import a *transient* engine failure degraded contributes a fabricated static size that carries `error: None`. In both cases the total must be returned (a floor beats a zero) with a diagnostic naming the specifier, and must **never** be written to the L1 aggregate cache (FR-026c) — a 30-second TTL is long enough for a number that describes a scheduling accident to become the file's reported size.
...
**FR-026c** (Critical) - **A result degraded by a transient engine failure must never be written to any cache.** A build that hits `BUILD_TIMEOUT`, panics, or loses the engine runtime (`timeout` / `panic` / `engine_gone`) leaves the pipeline no choice but the conservative static fallback — which carries `error: None` and a plausible byte count, and so is exactly the shape a cache stores without complaint. It describes the daemon's luck, not the package: caching it once makes a healthy 17,550-byte package report its 58-byte barrel for the rest of the cache generation. Every cache and memo therefore gates on the failure stage, not merely on `error`: the L1 memory and L2 disk import caches, the L1 aggregate file-size cache (whose combined build degrades the same way), and — by construction, since they are written only on the success path — the full-package and export-list build memos and the dependency-path index. A **deterministic** failure (`parse`, `link`, `resolve`, `output_shape`, `module_graph_limit`, a minification failure) is still cached: it is a fact about the code, and re-deriving it would cost another full build to learn the same thing.

  The **aggregate** cache needs one gate more than the stage scan, because a file total can be wrong without any stage having failed. A conservative per-import sum (FR-024a) that skipped a still-`loading` import, or that added the fabricated size of a transiently-degraded one, carries `error: None` and no transient stage of its own — the failure is recorded in the *import's* diagnostics, one level down. The daemon must therefore lift a summed import's transient stage into the aggregate's diagnostics, and must mark any sum that is missing an input as incomplete, so that neither can be cached. The sum must be structurally incapable of adding the bytes and dropping that signal: it is the same defect that produced the 58-byte cache poisoning, and it has now surfaced at three different levels of this design.

  **"Any cache" includes the extension's, and those are worse.** The extension persists two stores of its own — the import-cost history and the bundle-impact history, both in VS Code state — and neither has a TTL, a cache generation, or a Clear Caches command behind it. They keep **one row per identity**, so a fabricated size does not go stale there: it *replaces that import's real historical baseline permanently*, and every later trend ("was 17,550 B, now 58 B") is then computed against a number that never happened. Both must therefore gate on the same evidence the daemon uses:

  * an `ImportResult` may be recorded only if it carries no transient stage in its diagnostics — an `error` check is **not** sufficient and never was, since a transiently-degraded result carries `error: null` and a plausible size, which is exactly why this defect keeps recurring;
  * a file total may be recorded only if it is neither transiently degraded nor **incomplete**. `FileSizeDocumentResponse` therefore carries `incomplete` on the wire: the client cannot derive it, because the diagnostic that names a still-`loading` import is stage-tagged `file_size_fallback` — the same tag a deterministic per-import failure carries, which is a real fact and no reason to distrust the total.

  A total that fails this gate is still **shown** (a floor beats a blank, FR-024a) — labelled as an estimate, and with no delta against the previous run, since comparing an honest number to a floor invents a regression. It is simply never written down.
...
One exception to the "does not implement" list above, recorded so it is not mistaken for dead code: the daemon **retains a `package.json#sideEffects` glob matcher**, but it is *reporting-only and retention-neutral*. It never reaches Rolldown and cannot change what is retained or what size is reported. It survives solely on the **static-fallback path** — taken when the engine fails — where it drives the `side_effects` flag and the matched-path diagnostic, because Rolldown 1.1.5 does not expose its retention decisions and the fallback would otherwise lose that diagnostic entirely. See the I9 amendment in the bundler-redesign design.
