...
| Minification (DCE)         | `oxc_minifier` (v0.134.x)    | Dead code elimination, constant folding, branch pruning. Stable 0.x release line; acceptable for size estimation within 1-2% variance.                                                                                                 |
| Identifier mangling        | `oxc_mangler` (v0.134.x)     | Separate crate from `oxc_minifier` (split during the 0.12x refactor). Handles scope-aware variable renaming and base54 name generation. May re-merge into `oxc_minifier`; see Appendix C.                                              |
...
OXC Rust crates and the `oxc-parser` npm package use 0.x versions, but that does not mean they are alpha quality. OXC follows Rust and npm package versioning before a 1.0 line while publishing production-ready crates and packages. ImportLens treats the OXC 0.133.x line as stable for v1, uses caret ranges so compatible patch upgrades are available, and runs the integration suite against each OXC upgrade. Minifier output can differ from SWC by 1 to 2 percent; that variance is acceptable for inline size estimates. See constraint C-001 in Section 13.1.

### 9.4 Dependency Manifest (Baseline Versions)

> **This table is the authoritative source of truth for dependency baseline versions and compatible semver ranges.** All `Cargo.toml` and `package.json` files must use these baselines or the documented compatible ranges at project initialization. This prevents AI agents and developers from using outdated or deprecated packages. OXC versioning policy last audited: **4 June 2026.**

#### 9.4.1 Rust Crates (`Cargo.toml`)

| Crate             | Baseline Version | Semver Range | Stability       | Notes                                                                                                                                                                                      |
| ----------------- | ---------------- | ------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc_parser`      | 0.134            | `^0.134`     | ✅ Stable API    | Use the caret range; Cargo's caret semantics for 0.x stays within 0.133.x. All OXC monorepo crates must use the same version line.                                                         |
| `oxc_resolver`    | 11.19.2          | `^11.19`     | ✅ Stable        | Separate repo from OXC monorepo; versioned independently.                                                                                                                                  |
| `oxc_semantic`    | 0.134            | `^0.134`     | ✅ Stable API    | Must match `oxc_parser` version line.                                                                                                                                                      |
| `oxc_transformer` | 0.134            | `^0.134`     | ✅ Stable API    | TS/JSX stripping only. Does NOT tree-shake.                                                                                                                                                |
| `oxc_minifier`    | 0.134            | `^0.134`     | ✅ Stable API    | Test every compatible upgrade against the accuracy suite because minified output can shift across releases.                                                                                |
| `oxc_mangler`     | 0.134            | `^0.134`     | ✅ Stable API    | Separate crate from `oxc_minifier`; keep on the same OXC version line.                                                                                                                     |
| `oxc_codegen`     | 0.134            | `^0.134`     | ✅ Stable API    | Required for AST → string. Use `minify: true`.                                                                                                                                             |
| `oxc_allocator`   | 0.134            | `^0.134`     | ✅ Stable        | Arena allocator. Must match parser version line.                                                                                                                                           |
| `oxc_ast`         | 0.134            | `^0.134`     | ✅ Stable API    | Shared AST data structures. Must match parser version line.                                                                                                                                |
| `oxc_span`        | 0.134            | `^0.134`     | ✅ Stable        | Source locations. Must match parser version line.                                                                                                                                          |
| `oxc_syntax`      | 0.134            | `^0.134`     | ✅ Stable API    | Syntax metadata used by the parser and downstream OXC stages. Must match parser version line.                                                                                              |
...
| Package                              | Baseline Version | Category                               | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------ | ---------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc-parser`                         | 0.134            | `dependency`                           | NAPI binding. `package.json` must use `^0.134.0`; npm caret semantics for 0.x stays within 0.133.x. Must stay on the same OXC version line as the Rust-side `oxc_parser` crate because both are released from the same OXC monorepo on the same cadence. **NOT** `@oxc-parser/wasm` (deprecated).                                                                                                                                                                                                                                                                              |
| `@oxc-parser/binding-win32-x64-msvc` | 0.134            | `dependency` in the Windows VSIX build | Native parser binding required by `oxc-parser` at runtime for the `win32-x64` VSIX. Other platform VSIX builds must include the matching `@oxc-parser/binding-*` package for their target. Because pnpm stores dependencies behind junctions/symlinks and `vsce --no-dependencies` excludes `node_modules`, the VSIX packaging step stages physical copies of `oxc-parser`, `@oxc-project/types`, and the target binding, then invokes `vsce package` with a staged manifest containing no devDependencies.                                                                    |
...
| ------------------------ | ------------------------------------------------------------- | --------------------------------------------------------------- |
| `@oxc-parser/wasm` (npm) | Officially deprecated. No longer maintained.                  | `oxc-parser` (NAPI, `^0.134.0`)                                 |
...
**C-001:** OXC Rust crates and the `oxc-parser` npm package use 0.x versions, but those versions are not alpha releases. ImportLens treats the OXC 0.133.x line as stable and uses caret ranges (`^0.134` for Rust crates and `^0.134.0` for npm) so compatible patch upgrades are available. Size estimation accuracy of approximately plus or minus 2 percent remains acceptable for an inline hint tool. **Fallback strategy:** If `oxc_minifier` exhibits correctness regressions in the integration test suite after a compatible upgrade, the team must pin to the last known-good version and file an upstream issue. No release VSIX will ship with a minifier version that fails the integration suite. As a last resort, the daemon may skip minification entirely and report only raw + compressed sizes, with a `(no-minify)` suffix on decorations.

**C-002:** The `oxc-parser` npm package (NAPI binding, v0.134 baseline) uses native bindings and does NOT work in browser environments. The deprecated `@oxc-parser/wasm` package must not be used due to its deprecated status. For VS Code for the Web, the extension enters degraded mode with no parsing capability. The Rust-side `oxc_parser` crate version line and the npm `oxc-parser` package version line are released from the same OXC monorepo on the same cadence; both must stay on the same compatible 0.133.x line.
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `oxc_minifier`                            | Stable 0.133.x release line. Produces 1-2% variance from SWC.                                                                                     | OXC 0.134+ or 1.0 releases; minifier API or output changes.                                                                                        | Upgrade OXC crates as a coordinated batch; re-run integration suite to confirm no accuracy regressions.                                                                                                                           | Every OXC release     |
| `oxc_mangler`                             | Stable 0.133.x release line; separate crate from `oxc_minifier`.                                                                                  | Potential crate-boundary or API changes, including possible re-merge into `oxc_minifier`.                                                          | Minor import path changes if crates are merged.                                                                                                                                                                                   | Every OXC release     |
| `oxc_resolver`                            | Baseline 11.19.x. Separate repository (`oxc-project/oxc-resolver`), versioned independently from the OXC monorepo. Currently on major version 11. | Major version bump (e.g. 12.x); breaking changes to `ResolverOptions` or the `resolve()` API.                                                      | May require `Cargo.toml` update and code changes in `resolver.rs`. Upgrade separately from the OXC monorepo batch and run integration suite before merging.                                                                       | Each release          |
