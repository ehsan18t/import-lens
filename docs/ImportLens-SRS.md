...
| Bundler       | tsdown (Rolldown-based)                           | Produces single-file CommonJS output (`extension/dist/extension.cjs`) with an explicit `node20` target for VS Code 1.90 extension-host compatibility, while build/test/package infrastructure runs on Node.js 24 LTS.                         |
...
> **This table tracks the current resolved dependency versions and the intended upgrade policy.** OXC monorepo Rust crates are pinned as a coordinated stack; `Cargo.lock` and `pnpm-lock.yaml` provide reproducible builds between deliberate upgrade runs. Run `pnpm deps:update:oxc` when intentionally refreshing the OXC stack; use `pnpm deps:update:all` only for broad dependency refreshes. Re-run the dependency policy and `pnpm test:accuracy` suites after either path. OXC versioning policy last audited: **5 June 2026.**

#### 9.4.1 Rust Crates (`Cargo.toml`)

| Crate             | Current Resolved Version | Version Policy | Stability       | Notes                                                                                                                                                                                      |
| ----------------- | ------------------------ | -------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc_parser`      | 0.138.0                  | exact pin      | ✅ Stable API    | OXC monorepo crate. Must be upgraded in lockstep with the other OXC monorepo crates.                                                                                                      |
| `oxc_resolver`    | 11.22.0                  | exact pin      | ✅ Stable        | Separate repo from OXC monorepo; versioned independently and upgraded separately.                                                                                                          |
| `oxc_semantic`    | 0.138.0                  | exact pin      | ✅ Stable API    | Must match `oxc_parser` resolved version.                                                                                                                                                  |
| `oxc_transformer` | 0.138.0                  | exact pin      | ✅ Stable API    | TS/JSX stripping only. Does NOT tree-shake.                                                                                                                                                |
| `oxc_minifier`    | 0.138.0                  | exact pin      | ✅ Stable API    | Test every upgrade against the accuracy suite because minified output can shift across releases. The daemon uses the minifier result's scoping and private-member mappings for codegen.     |
| `oxc_codegen`     | 0.138.0                  | exact pin      | ✅ Stable API    | Required for AST -> string. Use `minify: true`.                                                                                                                                            |
| `oxc_allocator`   | 0.138.0                  | exact pin      | ✅ Stable        | Arena allocator. Must match parser resolved version.                                                                                                                                       |
| `oxc_ast`         | 0.138.0                  | exact pin      | ✅ Stable API    | Shared AST data structures. Must match parser resolved version.                                                                                                                            |
| `oxc_ast_visit`   | 0.138.0                  | exact pin      | ✅ Stable API    | AST visitor utilities used for syntax-aware bundle rewrite context such as object shorthand detection. Must match parser resolved version.                                                  |
| `oxc_span`        | 0.138.0                  | exact pin      | ✅ Stable        | Source locations. Must match parser resolved version.                                                                                                                                      |
| `oxc_syntax`      | 0.138.0                  | exact pin      | ✅ Stable API    | Syntax metadata used by the parser and downstream OXC stages. Must match parser resolved version.                                                                                          |
...
| `tsdown`                             | 0.22.3           | `devDependency`                        | Rolldown-based bundler. Output: single-file `extension/dist/extension.cjs` CommonJS bundle targeting Node 20 syntax for VS Code 1.90 extension-host compatibility.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `@types/node`                        | 22.15.3          | `devDependency`                        | Explicit Node ambient types for Node APIs used by ImportLens (`fs/promises`, `net`, `child_process`, `crypto`, `path`, and Node's built-in test runner). Build infrastructure runs on Node 24 LTS, but this ambient type baseline is not raised by build-tool-only upgrades.                                                                                                                                                                                                                                      |
| `@vscode/vsce`                       | 3.9.2            | `devDependency`                        | VSIX packaging and publishing. ImportLens stages a minimal physical package tree and invokes `@vscode/vsce package --target <platform>` from that staging directory.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| pnpm                  | 11.9.0                   | Package manager          | Pinned through `packageManager`/Corepack, GitHub Actions `PNPM_VERSION`, and Docker `PNPM_VERSION`. Requires Node.js 22.13+; ImportLens uses Node.js 24 LTS for all build infrastructure.                                                                                                               |
