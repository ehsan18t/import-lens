...
**FR-018a** (High) - **A package's non-JavaScript bytes are counted when they can be processed, and disclosed when they cannot.** A stylesheet, wasm module, or font that the package's own JavaScript entry imports is real shipped weight, and the Import Cost includes it (B2). The daemon must:
  ...
  * link every classified asset as an empty module so the JavaScript graph can be measured at all. Rolldown 1.1.5 removed CSS bundling, and a `.css` module reaching it fails the **entire build** at the link stage (`UNSUPPORTED_FEATURE`). Every package whose ESM entry does `import './styles.css'` — most UI kits — was therefore unmeasurable, which nobody could see while a failed build was still given a fabricated size;
...
| Package ships CSS its JavaScript entry imports                      | Measure the JavaScript graph normally and link the stylesheet as an empty module. Rolldown 1.1.5 cannot bundle CSS (`UNSUPPORTED_FEATURE`, at the link stage), so left to it the **whole build fails** — which, before FR-026d, was invisible because the failure was silently given a fabricated size. Bundle the reachable stylesheets into one artifact, compress it separately, and add it to the reported sizes (FR-018a). A sheet that cannot be processed reverts to an `uncounted_assets` disclosure naming the files, at Medium confidence. |
...
**Assets are intercepted by the PLUGIN, not emitted by Rolldown.** Rolldown 1.1.5 does not emit a CSS asset — it refuses the build outright at the **link** stage (`UNSUPPORTED_FEATURE: Bundling CSS is no longer supported`). So the plugin intercepts a classified non-JS module at `load`, captures it as an immutable byte snapshot with the fingerprint from that same read, and returns an empty module, which is what lets the JavaScript graph be measured at all. The pipeline then processes those snapshots after the build and folds their compressed sizes into the reported number (FR-018a). Any claim that an emitted asset is disclosed and the chunk measured beside it describes a mechanism this bundler does not have.
