...
6. Renders the size inline in the editor as confidence-styled inline hints, native inlay hints, end-of-line decorations, or code lens annotations
...
**FR-029** (Critical) - The extension must display size information inline by default through `importLens.display: "inlayHint"` and `importLens.inlineRenderer: "colored"`. The colored inline renderer uses decoration-backed inline text so individual imports can use confidence-specific colors. Native VS Code inlay hints remain available through `importLens.inlineRenderer: "native"` for users who prefer the platform inlay-hint renderer or screen-reader-first behavior. End-of-line text decorations remain available via `importLens.display: "standard"` or `importLens.display: "verbose"` for users who prefer line-end annotations.
...
| D-011 | Hybrid inline rendering                                                                   | VS Code native inlay hints are accessible and integrate with editor controls, but the API cannot assign arbitrary colors per hint. ImportLens therefore defaults to decoration-backed colored inline hints for confidence visibility while keeping native inlay hints available through `importLens.inlineRenderer: "native"`.                                                                                              | Native InlayHints only (rejected: no per-hint confidence colors); end-of-line decorations only (rejected: less inline and less accessible); CodeLens only (rejected: takes full line)            |
