...
- **Tier 2 — patch-only (tilde `~`):** a minor bump could break, so only patches flow automatically. Applies to the coordinated OXC stack (`~0.139.0`, all monorepo crates on one version) and `oxc_resolver` (`~11.23.0`), and to `papaya ~0.2`. See §9.3 and constraint C-001.
...
| ----------------- | ------------------------ | -------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oxc_parser`      | 0.139.0                  | `~` patch pin  | ✅ Stable API    | OXC monorepo crate. Must be upgraded in lockstep with the other OXC monorepo crates.                                                                                                       |
| `oxc_resolver`    | 11.23.0                  | `~` patch pin  | ✅ Stable        | Separate repo from OXC monorepo; versioned independently and upgraded separately.                                                                                                          |
| `oxc_semantic`    | 0.139.0                  | `~` patch pin  | ✅ Stable API    | Must match `oxc_parser` resolved version.                                                                                                                                                  |
| `oxc_transformer` | 0.139.0                  | `~` patch pin  | ✅ Stable API    | TS/JSX stripping only. Does NOT tree-shake.                                                                                                                                                |
| `oxc_minifier`    | 0.139.0                  | `~` patch pin  | ✅ Stable API    | Test every upgrade against the accuracy suite because minified output can shift across releases. The daemon uses the minifier result's scoping and private-member mappings for codegen.    |
| `oxc_codegen`     | 0.139.0                  | `~` patch pin  | ✅ Stable API    | Required for AST -> string. Use `minify: true`.                                                                                                                                            |
| `oxc_allocator`   | 0.139.0                  | `~` patch pin  | ✅ Stable        | Arena allocator. Must match parser resolved version.                                                                                                                                       |
| `oxc_ast`         | 0.139.0                  | `~` patch pin  | ✅ Stable API    | Shared AST data structures. Must match parser resolved version.                                                                                                                            |
| `oxc_ast_visit`   | 0.139.0                  | `~` patch pin  | ✅ Stable API    | AST visitor utilities used for syntax-aware bundle rewrite context such as object shorthand detection. Must match parser resolved version.                                                 |
| `oxc_span`        | 0.139.0                  | `~` patch pin  | ✅ Stable        | Source locations. Must match parser resolved version.                                                                                                                                      |
| `oxc_syntax`      | 0.139.0                  | `~` patch pin  | ✅ Stable API    | Syntax metadata used by the parser and downstream OXC stages. Must match parser resolved version.                                                                                          |
