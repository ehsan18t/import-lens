...
- [x] **P1-5. Superseded bulk registry refreshes surface fabricated per-target errors** — `server.rs:648-655` fills cancelled slots with `"worker did not return a result"`; daemon supersession is per-connection while the client guard (`registryRefresh.ts:128-136`) is per (uri, target) → a refresh for a different package.json surfaces stale/error hint state for intentionally superseded work.
  ...
  - **✅ FIXED.** `RegistryRefreshLifecycle` is now keyed **per source manifest** (a `HashMap<source, flag>`), mirroring `SwrRefreshLifecycle` / D9 — a new bulk block supersedes only the same-source block, so refreshing a second `package.json` no longer cancels the first's in-flight targets. A new optional `source` field (the client document key) threads through `RefreshRegistryHintsRequest`; absent source shares one bucket (pre-D11 behavior). Reproduces cold-cache only (warm block finishes before the next arrives). See **decision-log D11**. Tests: daemon cross-source isolation + same-source supersede + drop-cancels-all; extension request carries `source`.
