...
///
/// Past values are not recorded here. Each was set by the commit that moved the numbers, so
/// `git log -S analyzer_revision -- daemon/src/cache/key.rs` gives every one with the change that
